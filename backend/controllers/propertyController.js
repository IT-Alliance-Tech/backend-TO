...
// NEW: GET USER PROPERTIES - For landing page (PUBLIC - No Auth Required)
...
// NEW: GET SINGLE USER'S PROPERTIES
// Endpoint: GET /api/properties/owner/:userId
const getSingleUserProperties = async (req, res) => {
  try {
    ...
    // Base filter - only properties owned by this user
    const filter = {
      owner: userId,
      status: { $in: ["approved", "published"] },
    };
    ...
    // Apply tiered access based on authentication and subscription
    if (!req.user) {
      properties = props.map(formatGuestView);
      userAccessLevel = "guest";
    } else {
    ...
    console.error("Error in getSingleUserProperties:", err);
    return res.status(500).json({
      statusCode: 500,
      success: false,
      error: {
...
module.exports = {
  ...
  // uploadImages, // Uncomment if you add this function
};
